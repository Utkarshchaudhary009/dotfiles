...
- [ ] 3.3 Detect file-level conflicts (same tracked file changed both locally and in the incoming remote tree) before any commit/push, and refuse to overwrite local edits. Emit a clear recovery command.
...
- [ ] 3.5 Surface a structured `SyncResult` (status, actions taken, conflicts, recommended next command) and accept `--json` on `agenv sync` so agents and scripts can act on the same state the user sees.
